...
DWORD WINAPI CanMsgReadThreadProc_CAN_NSI(LPVOID pVoid)
{
    ...
    while (bLoopON)
    {
        ...
        if (pThreadParam->m_unActionCode == EXIT_THREAD)
        {
            bLoopON = false;
            break;
        }
...
/// \param         void
/// \return        S_OK if succeeded, else respective error code
/// \authors       Grégory Merhat
/// \date          19.04.2013 Created
///-------------------------------------------------------------------
static int nSetApplyConfiguration()
...
/// \date          12.10.2011 Created
///-----------------------------------------------------
HRESULT CDIL_CAN_NSI::CAN_GetCurrStatus(STATUSMSG& StatusData)
{
...
/// \date          12.10.2011 Created
///---------------------------------------------------------
HRESULT CDIL_CAN_NSI::CAN_SetAppParams(HWND hWndOwner)
{
...
HRESULT CDIL_CAN_NSI::CAN_SendMsg(DWORD dwClientID, const STCAN_MSG& sCanTxMsg)
{
    ...
    {
        if (nTransmitMessage(sendingMsg, dwClientID) == S_OK)
        {
            hResult = S_OK;
        }
    }
